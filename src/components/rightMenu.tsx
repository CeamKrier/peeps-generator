...
import { Accessories, BustPose, Face, FacialHair, Hair, AccessoryType, BustPoseType, FaceType, FacialHairType, HairType, SittingPoseType, StandingPose, StandingPoseType, SittingPose } from "react-peeps";
...
const RightMenu = () => {
    ...
    const { pickedAccessory, pickedBody, pickedFace, pickedFacialHair, pickedHair, pickedSection, scaleVector, isFrameTransparent } = state;
    ...
    useEffect(() => {
        ...
        keys.bodyKeys = [...Object.keys(BustPose), ...Object.keys(SittingPose), ...Object.keys(StandingPose)];
    ...
    const renderPiece = (piece: string) => {
        switch (pickedSection) {
            ...
            case "Body":
                return React.createElement(BustPose[piece as BustPoseType] || SittingPose[piece as SittingPoseType] || StandingPose[piece as StandingPoseType]);
            case "Hair":
        ...
        return () => {
            switch (pickedSection) {
                case "Accessories":
                    ...
                    break;
                case "Body":
                    updateBody(piece as BustPoseType & SittingPoseType & StandingPoseType);
                    break;
                case "Hair":
            ...
            return (
                <li key={index} className='pieceListItem' onClick={handlePieceItemClick(piece)}>
                    <div className='pieceListWrapper'>
                        <input className='pieceListRadioButton' type='radio' name={pickedSection} checked={isPieceChecked(piece)} readOnly />
    ...
    const pickedSectionObject = () => {
        switch (pickedSection) {
            ...
            case "Body":
                return [...Object.keys(BustPose), ...Object.keys(SittingPose), ...Object.keys(StandingPose)];
            case "Hair":
    ...
    const renderSelectedPieceSet = useMemo(() => {
        return (
            <div className='listWrapper'>
                <ul className={`pieceList ${pickedSection}`}>{renderPieceList(pickedSectionObject() as string[])}</ul>
        ...
        return (
            <div className={`frameOptionsWrapper ${!isFrameTransparent && "increaseFrameWrapperWidth"}`}>
                ...
                <div style={{ display: "flex", ...(!isFrameTransparent && { display: "none" }) }}>
                    <div className={`frameOptionButton ${isFrameTransparent && "deactiveFrameOptionButton"}`} onClick={updateFrameType(true)}>
                        transparent
                    </div>
                    <div className={`frameOptionButton ${!isFrameTransparent && "deactiveFrameOptionButton"}`} onClick={updateFrameType(false)}>
                        colorful
                    </div>
                </div>
